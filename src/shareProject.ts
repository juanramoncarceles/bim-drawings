...
export class ShareProject {
  ...
  shareFormErrorMsg: HTMLElement;
  confirmChangesBtns: HTMLElement;
  saveChangesBtn: HTMLElement;
  cancelChangesBtn: HTMLElement;
  waitingAnimation: HTMLElement;
  currentUsersContent: HTMLElement;
  currentUsersTable: HTMLElement;
  pendingUsersContent: HTMLElement;
  pendingUsersToAddTable: HTMLElement;
  shareNotificationContainer: HTMLElement;
  shareNotificationInput: HTMLInputElement;
  app: Application;
  ...
  // Permission ids pending to delete for the current project.
  pendingToRemove: string[] = [];
  // Emails to share with.
  pendingToAdd: string[] = [];

  constructor(shareProjectContainer: HTMLElement, App: Application) {
    ...
    this.currentUsersTable.addEventListener('click', e => {
      if ((e.target as HTMLElement).closest('[data-action]')) {
        ...
        if (actionBtn.dataset.action === 'stage') {
          this.stageMemberToRemove(actionBtn.parentElement as HTMLTableRowElement);
        } else { // e.target.dataset.action === 'unstage'
          this.unstageMemberToRemove(actionBtn.parentElement as HTMLTableRowElement);
        }
      }
    });
    ...
    this.pendingUsersToAddTable.addEventListener('click', e => {
      if ((e.target as HTMLElement).closest('[data-action]')) {
        this.unstageMemberToAdd((e.target as HTMLElement).closest('[data-action]').parentElement as HTMLTableRowElement);
      }
    });
   ...
   */
  setUpDialog(projectData: ProjectData) {
   ...
   * @param e The submit event of the form to stage members to add.
   */
  stageMemberToAdd(e: Event) {
    e.preventDefault();
    const emailAddress = ((e.target as HTMLFormElement).elements.namedItem('email') as HTMLInputElement).value;
    if (emailAddress !== '' && !this.pendingToAdd.includes(emailAddress) && this.projectData.permissions.find(p => p.emailAddress === emailAddress) === undefined) {
      ...
      ((e.target as HTMLFormElement).elements.namedItem('email') as HTMLInputElement).value = '';
   ...
   * @param element Table row HTML element with the data of the member to unstage. It should have a data attr with the email.
   */
  unstageMemberToAdd(element: HTMLTableRowElement) {
   ...
   * @param element Table row HTML element with the data of the member to delete. It should have a data attr with the permission id.
   */
  stageMemberToRemove(element: HTMLTableRowElement) {
    ...
    const actionBtn = element.querySelector('.action-btn') as HTMLElement;
   ...
   * @param element Table row HTML element with the data of the member to delete. It should have a data attr with the permission id.
   */
  unstageMemberToRemove(element: HTMLTableRowElement) {
   ...
   */
  addStagedMembers() {
    const addMembersPromises: Promise<gapi.client.Response<gapi.client.drive.Permission>>[] = [];
   ...
   */
  removeStagedMembers() {
    const removeMembersPromises: Promise<gapi.client.Response<{}>>[] = [];
